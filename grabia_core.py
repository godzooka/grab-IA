...
class GrabIAEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.scan_lock = threading.Lock() # Added for rate-limiting control
        self.stop_event = threading.Event()
        self.config = config
        
        # BUG FIX: Use the 'output' path from config if provided, otherwise default to "downloads"
        custom_path = config.get('output')
        if custom_path:
            self.output_dir = Path(custom_path).resolve()
        else:
    ...
    def _scan_single_item(self, item_id: str):
        ...
        # Pacing: Ensure we don't hammer the metadata API too fast
        # Adding a tiny 0.1s delay between metadata requests to prevent trigger-happy limiting
        time.sleep(0.1)
        ...
        try:
            ...
            with self.db_lock:
                with sqlite3.connect(self.db_path, timeout=30) as conn:
                    cursor = conn.execute(
                        "SELECT filename FROM files WHERE id=? AND status='completed'", 
                        (item_id,)
                    )
            ...
            # Back-off logic: If we hit an error, wait 2 seconds before the next scan thread tries.
            # This cools down the API connection without stopping the download workers.
            time.sleep(2.0)
        finally:
